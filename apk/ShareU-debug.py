...
apk_dir = os.path.join(currentdir, "app/build/outputs/apk/normal/debug")
...
def startBuildApk():
    logger.info("**************开始构建APK**************")
    cmd_build = "./gradlew clean  assembleDebug  --configure-on-demand --daemon --parallel --build-cache --stacktrace"
    builddata = {
        "msgtype": "markdown",
        "markdown": {
            "title": "ShareU编译失败",
            "text": "#### 编译失败-" + Branch + "(#" + BUILD_NUMBER + ")" \
                    + "\n  > ##### [查看日志](http://ci.flatincbr.com/job/Game-Quiz-debug/" + BUILD_NUMBER + "/console)"
        }
    }
...
if __name__ == "__main__":
    ...
    version_name = modiyProFile(gradle_properties_path)
    pkg_name, apk_path = startBuildApk()
    ...
    try:
        ...
        cms_add_content = add_info_to_cms(is_test=True, **playload)
        if cms_add_content["status"] != 1:
            raise Exception("信息上传到cms失败,信息为：" + json.dumps(cms_add_content))

        qrcode_url = makeqrcode(apk_dir=os.path.dirname(apk_path), url=apk_url, project=project, is_test=True,
                                build_num=BUILD_NUMBER)

        Data = {
            "msgtype": "markdown",
            "markdown": {
                "title": "ShareU测试包(#" + BUILD_NUMBER + ")",
                "text": "#### ShareU测试包(#" + BUILD_NUMBER + ")-" + Branch \
                        + "\n  > ##### 更新日志：" + Title \
        ...
        logger.info("第二次修改的内容：")
        version_name = modiyProFile(gradle_properties_path)
        commit_code_after(Branch=Branch, gradle_properties_path=gradle_properties_path, version_name=version_name,
                          build_num=BUILD_NUMBER)
